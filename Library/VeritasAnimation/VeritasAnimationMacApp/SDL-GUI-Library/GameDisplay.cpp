...
void display_image (const SDL_Surface *src, SDL_Surface *dest, int x, int y, bool update, SDL_Rect rect){
	...
	SDL_Rect dest_rect = {static_cast<Sint16>(x), static_cast<Sint16>(y), rect.w, rect.h};
...
SDL_Surface* createText (const string &a){
	...
	for (size_t i = 0; i < letters.size(); i++) {
		...
		SDL_Rect rect = {static_cast<Sint16>(x), 0, static_cast<Uint16>(letters[i]->w), static_cast<Uint16>(letters[i]->h)};
